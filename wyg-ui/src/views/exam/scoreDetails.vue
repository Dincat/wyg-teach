      ...
      <el-col :span="24">
        <el-card class="score-info">
          ...
          <el-row>
            <el-col :span="6">
              <div class="user-info">
                <span class="user-info-item" :title="examRecord.examinationName">考试名称：{{ examRecord.examinationName }}</span>
                <br>
        ...
        <el-card class="subject-list">
          <el-table
            ...
            <el-table-column :label="$t('table.subject.type')" min-width="90">
              <template slot-scope="scope">
                <dict-tag :options="dict.type.subject_types" :value="scope.row.subject.type"/>
              </template>
            </el-table-column>
            ...
            <el-table-column :label="$t('table.subject.answer')" min-width="120">
              <template slot-scope="scope">
                <span v-html="scope.row.subject.answer.answer"></span>
              </template>
            </el-table-column>
            <el-table-column :label="$t('table.examRecord.markStatus')" min-width="90">
              <template slot-scope="scope">
                <dict-tag :options="dict.type.mark_status" :value="scope.row.markStatus"/>
              </template>
            </el-table-column>
            <el-table-column :label="$t('table.answerCorrectType')" min-width="90">
              <template slot-scope="scope">
                <dict-tag :options="dict.type.answer_types" :value="scope.row.answerType"/>
              </template>
            </el-table-column>
...
export default {
  name: 'ScoreDetails',
  dicts: ['subject_types','mark_status','answer_types'],
