  ...
  <div class="app-container">
    <el-form :model="listQuery" ref="queryForm" :inline="true">
      <el-form-item :label="$t('table.subjectName')" prop="subjectName">
        <el-input :placeholder="$t('table.subjectName')" v-model="subject.listQuery.subjectName" style="width: 240px;"
                  size="small" clearable @keyup.enter.native="handleFilterSubject"/>
      </el-form-item>

      <el-form-item>
        <el-button size="mini" v-waves type="primary" icon="el-icon-search" @click="handleFilterSubject">{{
          $t('table.search') }}
        </el-button>
      </el-form-item>
    </el-form>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button size="mini" v-hasPermi="['exam:exam:subject:add']" class="filter-item" type="primary" icon="el-icon-check"
                   @click="handleCreateSubject">{{ $t('table.add') }}
        </el-button>
      </el-col>

      <el-col :span="1.5">
        <el-button size="mini" v-hasPermi="['exam:exam:subject:import']" class="filter-item" type="success" icon="el-icon-upload2"
                   @click="handleImportSubject">{{ $t('table.import') }}
        </el-button>
      </el-col>

      <el-col :span="1.5">
        <el-button size="mini"  v-hasPermi="['exam:exam:subject:export']" class="filter-item" type="success" icon="el-icon-download"
        ...
        <template slot-scope="scope">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-hasPermi="['exam:exam:subject']">
    ...
    <div class="pagination-container">
      <el-pagination v-show="subject.total>0" :current-page="subject.listQuery.pageNum" :page-sizes="[10,20,30,50]"
                     :page-size="subject.listQuery.pageSize" :total="subject.total" background
                     layout="total, sizes, prev, pager, next, jumper" @size-change="handleSubjectSizeChange"
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        fetchSubjectListById(this.subject.listQuery).then(response => {
          if (response.rows.length > 0) {
            response.rows.map(subject => {
              subject.type = parseInt(subject.type)
              subject.level = parseInt(subject.level)
            })
            this.subject.list = response.rows
          } else {
            this.subject.list = []
          }
          this.subject.total = parseInt(response.total)
          setTimeout(() => {
      ...
      handleUpdateSubject(row) {
        ...
        this.$router.push({
          path: `/exam/subjects/detail/${examinationId}-${row.subjectId}-${row.type}-0`
        })
      },
      // 查看题目
