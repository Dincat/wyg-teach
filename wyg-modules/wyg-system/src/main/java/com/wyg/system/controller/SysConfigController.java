...
public class SysConfigController extends BaseController
{
    ...
    @GetMapping(value = "/configValue/{configKey}")
     ...
     */
    //@RequiresPermissions("system:config:list")
    @PostMapping("/noauth/web/list")
    public AjaxResult list(@RequestBody String[] configKeys)
    {
        List<SysConfig> list=new ArrayList<>();
        if(configKeys==null || configKeys.length<1) return AjaxResult.success(list);

        list = configService.selectConfigByKeys(configKeys);
